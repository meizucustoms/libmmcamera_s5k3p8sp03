...
// 65-entry noise profile, stores std dev. Will take sqrt, combine with edge_softness and divided by (1<<13) to form final noise table
// Default value: { 10, 134, 258, 382, 506, 630, 754, 877, 1000, 1119, 1237, 1356, 1474, 1593, 1712, 1831,
//              1949, 2068, 2187, 2306, 2424, 2543, 2662, 2781, 2899, 3018, 3136, 3255, 3374, 3493, 3611, 3700,
//              3789, 3789, 3789, 3789, 3789, 3789, 3789, 3789, 3789, 3789, 3789, 3789, 3789, 3789, 3789, 3789,
//              3789, 3789, 3789, 3789, 3789, 3789, 3789, 3789, 3789, 3789, 3789, 3789, 3789, 3789, 3789, 3789, 3789}
...
// normalized anchor location for 1d RNR table.
...
// enable single bpc when BPC module is off. Enabled for low power use cases.
// Default value: 0
// Data range: 0 or 1
// Effect:
//
...
//
//=============================================================================
#define ABFV34_NOISESTD_LENGTH  (64+1)
#define ABF34_RADIAL_POINTS     5

typedef struct
{
    trigger_point_type  ABF34_trigger;
    unsigned short      bpc_fmax;
    unsigned short      bpc_fmin;
    unsigned short      bpc_maxshft;
    unsigned short      bpc_minshft;
    unsigned short      bpc_offset;
    unsigned char       blk_pix_matching_rb;
    unsigned char       blk_pix_matching_g;
    unsigned short      distance_ker[2][3];
    unsigned int        noise_std_lut_level[ABFV34_NOISESTD_LENGTH];
    unsigned short      curve_offset[ABF3_CHANNEL_MAX];

    float               filter_strength_lo[2];
    float               filter_strength_hi[2];
    unsigned int        filter_strength_anchor_pt_lo;
    unsigned int        filter_strength_anchor_pt_hi;

    float               edge_softness;
    int                 base_table[2][ABF34_RADIAL_POINTS];
 } chromatix_abf34_core_type;

 typedef struct
 {
    unsigned char       cross_channel_process_en;
    float               anchor_table[ABF34_RADIAL_POINTS];
    unsigned int        horizontal_center;
    unsigned int        vertical_center;
 } chromatix_abf34_reserve_type;
...
{
    int                             abf34_enable;
    int                             single_bpc_en; // ABF34 Single BPC En
    int                             control_enable;
    tuning_control_type             control_abf3;
    trigger_point_type              abf3_hysteresis_pt;
    trigger_point_type              sbpc_hysteresis_pt;
    int                             abf34_snapshot_hyst_en_flag;
    int                             sbpc_snapshot_hyst_en_flag;
    chromatix_abf34_core_type       abf34_config[MAX_LIGHT_TYPES_FOR_SPATIAL];
    chromatix_abf34_reserve_type    reserve_data;
} chromatix_ABFV34_type;
...
//=============================================================================
#define GIC2_NOISESTD_LENGTH (64+1)

typedef struct
{
    trigger_point_type  GIC_trigger;
    unsigned int        noise_std_lut_level0[GIC2_NOISESTD_LENGTH];
    unsigned int        thin_line_noise_offset;
    float               filter_strength;
    float               denoise_scale;
} chromatix_gic_v2_core_type;
...
typedef struct
{
    ...
    trigger_point_type          gic_hysteresis_pt;
    chromatix_gic_v2_core_type  gic_config[MAX_LIGHT_TYPES_FOR_SPATIAL];
} chromatix_GIC_v2_type;
...
typedef struct
{
    int Hot_Pixel_Correction_disable;  // 1u; Disable hot pixel correction for DSBPC
    int Cold_Pixel_Correction_disable; // 1u; Disable cold pixel correction for DSBPC
} chromatix_PDPC_DSBPC_reserve;

typedef struct
{
    trigger_point_type  PDPC_trigger;
    unsigned short      Fmax;               //8uQ6; Bad pixel maximum threshold factor, Q6 unsigned -  (change in light conditions)   - default: 64
    unsigned short      Fmin;               //8uQ6; Bad pixel minimum threshold factor, Q6 unsigned -  (change in light conditions)   - default: 64
    unsigned int        bp_offset_g;        // T1(all)  //15u; Upper/Lower bad pixel threshold offset for green pixels         - default: 16384
    unsigned int        bp_offset_rb;       // T1(all)  //15u; Upper/Lower bad pixel threshold offset for red/blue pixels      - default: 16384
    unsigned int        bp_offset_g_T2;     // T2       //15u; Upper/Lower bad pixel threshold offset for green pixels         - default: 16384
    unsigned int        bp_offset_rb_T2;    // T2       //15u; Upper/Lower bad pixel threshold offset for red/blue pixels      - default: 16384
} chromatix_PDPC_DSBPC_core;
...
typedef struct
{
    ...
    int                             pdaf_dsbpc_en;
    int                             control_enable;
    tuning_control_type             control_PDPC;
    trigger_point_type              dsbpc_hysteresis_trigger;
    chromatix_PDPC_DSBPC_core       PDPC_config[MAX_LIGHT_TYPES_FOR_SPATIAL];
    chromatix_PDPC_DSBPC_reserve    PDPC_reserve;
} chromatix_PDPC_DSBPC_type;
...
typedef struct
{
    ...
    trigger_point_type          skinnr_hysteresis_trigger;
...
// Applicable chipset(s): HW version 8996, 8998.
...
// Applicable chipset(s): HW version 8996.
...
// Applicable chipset(s): HW version 8996, 8998.
// Effect: The smaller value means that more pixels will be detected as Cold Pixel, and
//   more false detections will be included. A larger value means that fewer pixels will be
//   detected as Cold pixels, so some real cold pixels may be missed.
//
// variable name: BPC_Offset.
...
typedef struct
{
    ...
    int     master_curve_core[129];          // 14s
    int     master_scale_core[129];          // 14s
    int     shift_curve_core[129];           // 14s
    int     shift_scale_core[129];           // 14s
} chromatix_LTM_core;
...
typedef struct
{
    ...
    int                     curves_from_core_section_en;
...
typedef struct
{
    ...
    trigger_point_type                          abf2_hysteresis_trigger;
...
typedef struct
{
    ...
    tuning_control_type     control_gic;
    trigger_point_type      gic_hysteresis_trigger;
...
typedef struct
{
    ...
    trigger_point_type  bpc_hysteresis_trigger;
    ...
    trigger_point_type  bcc_hysteresis_trigger;
...
// Enable flag for whole module, this will enable Color Conversion and Chroma Enhacement blocks
...
typedef struct
{
    ...
    chromatix_color_conversion_type     LED1_color_conversion;              // LED1  (309)
    chromatix_color_conversion_type     LED2_color_conversion;              // LED2  (309)
...
typedef struct
{
    ...
    sce_cr_cb_triangle_set      origin_triangles_outdoor;
    sce_cr_cb_triangle_set      destination_triangles_outdoor;
    sce_shift_vector            shift_vector_outdoor;
} chromatix_SCE_type;
...
typedef struct
{
    ...
    chromatix_GTM_core      gtm_core_v2;
    chromatix_GTM_reserve   gtm_reserve_v2;
} chromatix_GTM_v2_exp_type;

typedef struct
{
    trigger_point_type          gtm_v2_trigger;
    chromatix_GTM_v2_exp_type   gtm_exp_data[MAX_SETS_FOR_TONE_NOISE_ADJ];
} chromatix_GTM_v2_rgn_type;
...
typedef struct
{
    ...
    chromatix_GTM_v2_rgn_type   gtm_v2[MAX_SETS_FOR_GTM_EXP_ADJ];    // 6 sets
} chromatix_GTM_v2_type;
typedef struct
{
    int                 gtm_enable;
...
typedef struct
{
    ...
    //=========================================================================
    // PDPC ( PDAF Pixel Correction )
    //=========================================================================
    chromatix_PDPC_DSBPC_type               chromatix_PDPC_data;
